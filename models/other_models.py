...
class StudentsRefund(models.Model):
    ...
    def unlink(self):
        for rec in self:
            ...
            return super(StudentsRefund, rec).unlink()
...
class BaseStudents(models.Model):
    _inherit = 'op.student'

    @api.model
    def create(self, vals):
        print('create method')
        record = super(BaseStudents, self).create(vals)
        record_name = vals.get('name', '')
        self.env['audit.trail'].sudo().create({
            'name': 'Create Record',
            'model': self._name,
    ...
    def write(self, vals):
        for record in self:
            ...
            print(old_vals, 'old vals')
            result = super(BaseStudents, record).write(vals)
    ...
    def unlink(self):
        ...
        return super(BaseStudents, self).unlink()
...
class StudentDiscountReport(models.Model):
    ...
    def create(self, vals):
        ...
        record_name = vals.get('student_id', '')
        self.env['audit.trail'].sudo().create({
            'name': 'Create Record',
            'model': self._name,
            'record_id': record.id,
    ...
    def write(self, vals):
        for record in self:
            ...
            self.env['audit.trail'].sudo().create({
                ...
                'changed_value': new_vals,
                'description': self._description,
                'record_name': record.student_id.name,
                'changes': f"Old: {old_vals}\nNew: {new_vals}",
            })
    ...
    def unlink(self):
        self.env['audit.trail'].sudo().create({
            ...
            'action': 'unlink',
            'description': self._description,
            'record_name': self.student_id.name,
            'changes': 'Record deleted',
        })
